...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    if (c == '!') {
        // Caractere '!' está no bloco 1, com dados organizados linha a linha.
        index = 1 * 8;
        for (uint8_t row = 0; row < 8; ++row) {
            uint8_t line = font[index + row];
            for (uint8_t col = 0; col < 8; ++col) {
                bool pixel_on = (line & (1 << col)) != 0;
                ssd1306_pixel(ssd, x + col, y + row, pixel_on);
            }
        }
    }
    else if (c >= '0' && c <= '9') {
        // Dígitos: '0' está no bloco 2, '9' no bloco 11
        index = (c - '0' + 2) * 8;
        for (uint8_t col = 0; col < 8; ++col) {
        ...
        // Letras maiúsculas: 'A' no bloco 12, 'Z' no bloco 37
        index = (c - 'A' + 12) * 8;
        for (uint8_t col = 0; col < 8; ++col) {
            uint8_t line = font[index + col];
            for (uint8_t row = 0; row < 8; ++row) {
        ...
        // Letras minúsculas: 'a' no bloco 38, 'z' no bloco 63.
        // Esses caracteres estão invertidos verticalmente na fonte, então invertemos os bits.
        index = (c - 'a' + 38) * 8;
        for (uint8_t col = 0; col < 8; ++col) {
            uint8_t line = font[index + col];
            for (uint8_t row = 0; row < 8; ++row) {
                bool pixel_on = (line & (1 << (7 - row))) != 0;
                ssd1306_pixel(ssd, x + col, y + row, pixel_on);
            }
        }
    }
    else {
        // Caractere não reconhecido; desenha o bloco "Nothing" (índice 0)
        for (uint8_t col = 0; col < 8; ++col) {
